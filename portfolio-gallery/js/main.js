...
;(function(window) {
	...
	IsoGrid.prototype._expandSubItems = function(item) {
		var self = this,
			...
			subItems = [].slice.call(itemLink.querySelectorAll('.layer')).sort(function (a, b) {
				var oa = parseInt(a.getAttribute('data-stack-order'), 10);
				var ob = parseInt(b.getAttribute('data-stack-order'), 10);
				if (isNaN(oa)) oa = 0;
				if (isNaN(ob)) ob = 0;
				return oa - ob;
			}),
	...
	IsoGrid.prototype._collapseSubItems = function(item) {
		...
		[].slice.call(itemLink.querySelectorAll('.layer')).sort(function (a, b) {
			var oa = parseInt(a.getAttribute('data-stack-order'), 10);
			var ob = parseInt(b.getAttribute('data-stack-order'), 10);
			if (isNaN(oa)) oa = 0;
			if (isNaN(ob)) ob = 0;
