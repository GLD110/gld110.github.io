import * as THREE from 'three';
    ...
    function applyEmbedTheme(theme) {
        ...
        const bg = isLight ? '#f6f9ff' : '#050505';
        params.bg = bg;
        scene.background = new THREE.Color(bg);
        document.documentElement.setAttribute('data-theme', isLight ? 'light' : 'dark');
    }

    // Sync with parent dark/light toggle.
    window.addEventListener('message', (event) => {
        if (!event.data || event.data.type !== 'theme-change') return;
        applyEmbedTheme(event.data.theme);
    });

    // Apply current html data-theme on boot.
    applyEmbedTheme(document.documentElement.getAttribute('data-theme') === 'light' ? 'light' : 'dark');
    ...
    textureLoader.load('./assets/my-photo.jpg', (tex) => {
